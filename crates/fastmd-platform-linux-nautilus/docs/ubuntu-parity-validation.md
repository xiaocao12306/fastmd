...
- the Ubuntu shell now keeps the saved cross-session review-signoff artifact paths, freshness booleans, and reviewer metadata in hidden shell metadata, so reviewers can inspect whether the umbrella Ubuntu parity-evidence item is merely captured, awaiting first review, stale after newer captures, or actually ready to close without changing visible preview copy
